...
#define RECV_SIZE_BUFFER 16384
#define REQUEST_SIZE_BUFFER 16384
#define API_BODY_SIZE_BUFFER 16000
...
bool network_init(uint16_t startPort, uint16_t endPort, EndCallback endCall, uint16_t socketConnectTimeout, uint16_t socketResponseTimeout){
    ...
    recvBuffer = (char *) calloc(RECV_SIZE_BUFFER, sizeof(char));

    if(recvBuffer == NULL){
        printf("Cannot allocate memory for recvBuffer\n");
    ...
    requestBuffer = (char *) calloc(REQUEST_SIZE_BUFFER, sizeof(char));

    if(requestBuffer == NULL){
        printf("Cannot allocate memory for requestBuffer\n");
...
bool network_connectToSocket(char * hostname, int port, uint16_t * outgoingPort){
    ...
    mySocket->setRecvBuffer(RECV_SIZE_BUFFER);
...
bool network_get_completion(char * hostname, int port, char * api_key, char * model, char * message, float temperature, COMPLETION_OUTPUT * output){
    ...
    memset(requestBuffer, 0, REQUEST_SIZE_BUFFER);
    snprintf(requestBuffer, REQUEST_SIZE_BUFFER, API_CHAT_COMPLETION, api_key, actual_body_size, api_body_buffer);
    //puts(requestBuffer);

    memset(recvBuffer, 0, RECV_SIZE_BUFFER);
    int actualRequestBufferLength = strlen(requestBuffer);
    bool status = network_send_receive(hostname, port, requestBuffer, actualRequestBufferLength, recvBuffer, RECV_SIZE_BUFFER, &output->outPort);

    output->error = COMPLETION_OUTPUT_ERROR_OK;
    output->rawData = recvBuffer;

    if(status){
        //puts(recvBuffer);

        //Find if contains error
        char * errorPtr = strstr(recvBuffer, "\"error\": {");

        if(errorPtr){
            //fprintf(stderr, "Found error\n");
            char * errorMsgKeyPtr = strstr(recvBuffer, "\"message\": \"");
            ...
            char * prompt_token_ptr = strstr(recvBuffer, "\"prompt_tokens\":");
            ...
            char * completion_token_ptr = strstr(recvBuffer, "\"completion_tokens\":");
            ...
            char * content_ptr = strstr(recvBuffer, "\"content\":");
